...
const Age = () => {
  ...
  return (
    <Layout title={t(`symbols.wonders.title`)} container>
      <table className="border-t border-gray-200 table-auto">
        <tbody>
          {(
            t('symbols.wonders.items', { returnObjects: true }) as {
              title: string;
              desc: string;
              image: string;
            }[]
            ...
            <tr key={image}>
              <td className="py-6 align-top border-b border-gray-300">
                <div className="w-14 h-36">
                  <img
                    className="object-contain left w-14 h-36"
                    src={`/images/symbols/wonders/${image}`}
                    alt={desc}
                  />
                </div>
              </td>
